#!/usr/bin/env python3
"""orze watchdog: continuously monitors an orze instance for stuck/zombie/
deadlock conditions and self-heals the ORCHESTRATOR PROCESS.

Platform-level tool — only diagnoses and fixes orze-runtime bugs (process
crashed, log stalled, zombies, stuck claims, low disk). Never touches
project-specific scripts (train scripts, eval scripts, etc.) — that work
belongs to the `engineer` role.

Reads orze.yaml for configuration. Runs forever alongside the orchestrator.

Usage:
    python -m orze_pro.agents.watchdog -c orze.yaml
"""
...
# ─── Defaults (overridable via orze.yaml watchdog section) ──────────────────
DEFAULTS = {
    "check_interval": 60,
...
def load_config(config_path: str) -> dict:
    """Load orze.yaml and extract watchdog settings."""
    with open(config_path, encoding="utf-8") as f:
        cfg = yaml.safe_load(f)
    wd_cfg = cfg.get("watchdog", {})
    merged = {**DEFAULTS, **wd_cfg}
    merged["_orze"] = cfg
    return merged
...
def check_orze_alive(cfg):
    ...
    results_dir = Path(orze_cfg.get("results_dir", "orze_results"))
...
def check_disk_space(cfg):
    ...
    issues = []
    results_dir = Path(cfg["_orze"].get("results_dir", "orze_results"))
    try:
...
def save_issue(issue, cfg):
    issues_dir = Path(cfg["_orze"].get("results_dir", "orze_results")) / "watchdog_issues"
...
def spawn_claude_fix(issue, cfg):
    ...
    prompt = f"""You are the Orze platform watchdog. An issue was detected in the running Orze system.
...
"""
    ...
    issues_dir = results_dir / "watchdog_issues"
    issues_dir.mkdir(parents=True, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(description="Orze watchdog agent")
    ...
    logger.info("=" * 60)
    logger.info("Orze watchdog started")
