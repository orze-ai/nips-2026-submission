...
def build_evolution_context(results_dir: Path, ideas_path: Path,
    ...
    # Top leaderboard — render per-competition so the LLM sees coverage
    # across the comp portfolio, not a single-comp top-N. Reuses
    # orze.core.medal so medal-tier semantics stay consistent everywhere.
    ...
    if lb_entries:
        ...
        for entry in lb_entries:
            ...
            cid = em.get("competition_id") or entry.get("competition_id") or "?"
            existing = per_comp.get(cid)
            if existing is None or medal_rank(em) > medal_rank(
                    existing.get("eval_metrics", {}) or {}):
                per_comp[cid] = entry
        lines.append("## Top per Competition (medal-tier first)\n")
        for cid, entry in sorted(per_comp.items()):
            em = entry.get("eval_metrics", {}) or {}
            medal = em.get("medal", "none")
            score = em.get(primary, entry.get("metric_value", "?")) if primary else "?"
            lines.append(f"- {cid}: {entry.get('idea_id','?')} "
                         f"medal={medal} {primary}={score}")
        lines.append("")
    ...
    # Family distribution from lake (canonical location: results_dir/idea_lake.db)
...
def _parse_ideas_from_output(output: str, results_dir: Path,
    ...
    from orze_pro.agents.research import parse_llm_ideas
...
def run_evolution_cycle(cfg: dict, results_dir: Path, ideas_path: Path,
    ...
    from orze_pro.agents.research import format_idea_markdown, append_ideas_to_md
    ideas_md = []
    for idea in ideas:
...
def main():
    ...
    # Always exit 0 for a completed cycle — returning 0 ideas / making no
    # code changes is a valid graceful outcome (nothing to evolve this
    # round, plateau-trigger conditions not met, all proposed changes
    # already applied). The inner run_evolution_cycle returns 0 in every
    # quiet no-op path and logs its decision. Forcing exit 1 here turned
    # each quiet cycle into a misleading "cycle failed" warning.
    sys.exit(0)
