        ...
        if metric_path:
            ...
            if total > 10:
                top_threshold = conn.execute(
                    "SELECT json_extract(eval_metrics, ?) as val FROM ideas "
                    "WHERE status = 'completed' AND json_valid(eval_metrics) "
                    "AND json_extract(eval_metrics, ?) IS NOT NULL "
...
def run_meta_research(cfg: dict, results_dir: Path,
    ...
    # 2. Read current research rules.
    #
    # With the skills-based refactor, the research role composes from
    # [@sop:research_base, ./RESEARCH_RULES.md]. The project-specific
    # half lives at <project>/.orze/rules/RESEARCH_RULES.md and is what meta-research
    # edits — the bundled @sop:research_base is static and read-only.
    # Fallback: search for any project-local .md entry in a research role's
    # skills list whose name contains "research" or "rules".
    rules_file: Path | None = None
    default_path = results_dir.parent / ".orze" / "rules" / "RESEARCH_RULES.md"
    # Fallback to legacy location
    if not default_path.exists():
        default_path = results_dir.parent / "RESEARCH_RULES.md"
    if default_path.exists():
        rules_file = default_path
    else:
