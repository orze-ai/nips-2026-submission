...
from orze_pro.agents.research_context import (
    build_context,
    get_existing_idea_ids,
)
from orze_pro.agents.research_llm import (
...
def _load_dead_configs(results_dir: Path) -> list:
    """Load proven-dead config patterns from .orze/rules/RESEARCH_RULES.md YAML block."""
...
def _filter_dead_configs(ideas: list, results_dir: Path) -> tuple:
    """Remove ideas matching proven-dead config patterns. Returns (kept, rejected)."""
    dead_patterns = _load_dead_configs(results_dir)
    ...
    for idea in ideas:
        cfg = idea.get("config", {})
        is_dead = any(
            all(cfg.get(k) == v for k, v in pat.items())
            for pat in dead_patterns
            if isinstance(pat, dict)
        )
        (rejected if is_dead else kept).append(idea)
...
def parse_llm_ideas(response: str, results_dir: Path, cycle: int) -> list:
    ...
    if json_ideas:
        for i, item in enumerate(json_ideas):
            ...
            # Validate approach_family against known set (optional — module
            # dropped in orze v4 lite refactor; passthrough if unavailable).
            raw_family = item.get("approach_family", "").lower()
            try:
                from orze.engine.family_guard import APPROACH_FAMILIES, infer_approach_family  # type: ignore
                if raw_family not in APPROACH_FAMILIES:
                    raw_family = infer_approach_family(
                        item["config"], item.get("category", ""))
            except ImportError:
                if not raw_family:
                    raw_family = item.get("category", "architecture").lower()
            ideas.append({
                "idea_id": idea_id,
                "title": item["title"],
...
# ---------------------------------------------------------------------------
#  Retrospection compression
# ---------------------------------------------------------------------------

_RETRO_MAX_CHARS = 50_000  # ~12k tokens — keep prompt under budget

def _compress_retrospection(text: str) -> str:
    """Compress retrospection to fit within token budget.

    Strategy: keep the first section (summary/overview) and the last section
    (most recent insights) in full.  Collapse the middle into a compact
    per-experiment one-liner.
    """
    if len(text) <= _RETRO_MAX_CHARS:
        return text
    ...
    for i, line in enumerate(lines):
        if (line.startswith("#") or line.startswith("==")) and i > cur:
            sections.append((cur, i))
            cur = i
    sections.append((cur, len(lines)))

    if len(sections) <= 2:
        # No clear sections — just keep head + tail
        head = "".join(lines[:200])
        tail = "".join(lines[-200:])
        return (head
                + "\n\n[... retrospection compressed: middle omitted ...]\n\n"
                + tail)

    # Keep first and last sections in full, compress middle
    first = "".join(lines[sections[0][0]:sections[0][1]])
    last = "".join(lines[sections[-1][0]:sections[-1][1]])
    mid_lines = []
    for s_start, s_end in sections[1:-1]:
        header = lines[s_start].strip()
        # Extract first non-empty content line as summary
        summary = ""
        for ln in lines[s_start+1:s_end]:
            stripped = ln.strip()
            if stripped and not stripped.startswith("---"):
                summary = stripped[:120]
                break
        mid_lines.append(f"- {header}: {summary}")
    ...
    if len(compressed) > _RETRO_MAX_CHARS:
        compressed = compressed[:_RETRO_MAX_CHARS] + "\n[... truncated ...]\n"
...
def _load_knowledge_dir(knowledge_dir: Path) -> str:
    """Concatenate every ``*.md`` under ``knowledge_dir`` with a per-file header.

    Returns the empty string when ``knowledge_dir`` is missing, not a
    directory, or contains no markdown files.  Files are sorted by name so
    output is deterministic across runs (test-friendly).  Each file is
    prefixed with ``# {filename}`` so the LLM can attribute findings.
    Read errors on individual files are logged at WARNING and the file is
    skipped — one unreadable file must not poison the whole context.
    """
    ...
    for p in sorted(knowledge_dir.glob("*.md")):
        try:
            ...
            logger.warning("  Could not read knowledge file %s: %s", p, e)
            continue
        if not body:
            continue
...
def run_research_cycle(
    ...
    existing_ids = get_existing_idea_ids(ideas_path, lake_db_path=lake_db_path)
        ...
        if retro_path.exists():
            try:
                ...
                if retro_content:
                    ...
                    retro_content = _compress_retrospection(retro_content)
                    logger.info("  Compressed retrospection to %d chars",
                                len(retro_content))
            except OSError as e:
                logger.warning("  Could not read retrospection file: %s", e)

    # 2c. Load all knowledge/*.md so any role that drops a finding into
    # ``results/knowledge/`` automatically reaches research without per-
    # project wiring.  Concatenated AFTER the (already-compressed)
    # retrospection so retrospection still gets prompt priority; each file
    # carries a ``# {filename}`` header so the LLM can attribute claims.
    if knowledge_dir is not None:
        kdir = Path(knowledge_dir)
        knowledge_content = _load_knowledge_dir(kdir)
        if knowledge_content:
            # Skip retrospection.md if it's the same file already loaded
            # above — avoid duplicating content into the prompt.
            retro_basename = (Path(retrospection_file).resolve()
                              if retrospection_file else None)
            kept_parts: list[str] = []
            for chunk in knowledge_content.split("\n\n# "):
                first = chunk if chunk.startswith("# ") else "# " + chunk
                # First line is "# {filename}" — extract filename
                fname = first.split("\n", 1)[0][2:].strip()
                if (retro_basename is not None
                        and (kdir / fname).resolve() == retro_basename):
                    continue
                kept_parts.append(first)
            extra = "\n\n".join(kept_parts).strip()
            if extra:
                logger.info("  Loaded knowledge from %s (%d files, %d chars)",
                            kdir, len(kept_parts), len(extra))
                # Append after retrospection so retrospection keeps
                # priority position; separator makes provenance visible.
                if retro_content:
    ...
    # 5b. Filter out dead configs (programmatic enforcement of RESEARCH_RULES.md)
    ideas, rejected = _filter_dead_configs(ideas, results_dir)
    if rejected:
        logger.info("  Filtered %d dead-config ideas: %s",
                     len(rejected), [r["idea_id"] for r in rejected])
    if not ideas:
        logger.warning("All ideas filtered as dead configs — nothing to append")
        return 0
...
""",
    ...
    parser.add_argument("--knowledge-dir", default="",
                        help="Directory of *.md knowledge files (e.g. results/knowledge/) "
                             "to auto-include alongside retrospection. Any role that drops "
                             "a finding here flows into research context with no extra wiring.")
    ...
    results_dir = Path(args.results_dir or cfg.get("results_dir", "orze_results"))
    ...
    # Always exit 0 for a completed cycle — returning 0 ideas is a valid
    # graceful outcome (LLM returned unparseable output, all ideas filtered
    # as dead configs, dry-run, etc). Those paths inside run_research_cycle
    # already log at WARNING and return 0. Unhandled exceptions propagate
    # and crash with a non-zero exit naturally, so exit 1 here only added
    # noise — every "no ideas this cycle" showed up as a scary cycle
    # failure in orze_*.log even though the role ran to completion.
    sys.exit(0)
