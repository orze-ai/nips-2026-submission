...
Usage:
    python -m orze_pro.agents.bot -c orze.yaml
"""
...
def cmd_help(msg: Message, cfg: dict) -> str:
    return (
        ...
        "/think — trigger creative reasoning (paradigm shift analysis)\n"
        "/help — this message\n\n"
...
def cmd_think(msg: Message, cfg: dict) -> str:
    """Manually trigger the thinker role for creative paradigm-shift reasoning."""
    from orze.core.config import orze_path
    trigger = orze_path(cfg, "triggers", "_trigger_thinker")
    trigger.write_text(f"user requested via /think\n", encoding="utf-8")
    return ("Thinker triggered. It will run on the next orchestrator cycle "
            "and write a paradigm report to _paradigm_report.txt.")
...
def _invoke_script(message: str, cfg: dict) -> str:
    ...
    orze_cfg = cfg["_orze"]
    template_vars = {
        "results_dir": orze_cfg.get("results_dir", "orze_results"),
        "ideas_file": orze_cfg.get("ideas_file", "ideas.md"),
        "message": message,
    }
    for a in args:
